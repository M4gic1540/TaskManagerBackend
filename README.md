# ASDP Ticketera — Backend (Django + DRF)

Sistema de gestión de tickets IT con 3 roles (Administrador, Técnico,
Usuario), JWT auth, y arquitectura en capas siguiendo Repository /
Service Layer / Factory / Strategy / Observer / Specification.

## Stack

- Python 3.12, Django 6.0, Django REST Framework 3.17
- SQLite (dev) — cambiar `DATABASES` en `config/settings.py` para Postgres
- JWT vía `djangorestframework-simplejwt` (access 15 min, refresh 7 días, rotación + blacklist)
...
- **Repository**: los `Service` nunca llaman `Ticket.objects` directo. Permite
  testear lógica de negocio con un `FakeRepository` en memoria (ver `tickets/tests/`),
  sin base de datos — Dependency Inversion Principle en la práctica.
...
- **Observer**: `EventBus` desacopla efectos secundarios (auditoría, notificaciones)
  de la lógica principal. El `TicketService` publica eventos; no sabe quién
  escucha ni qué hacen los observers.
- **Specification**: filtros de listado (`status`, `priority`, `category`, `search`)
  y el scoping por rol (Usuario ve lo suyo, Técnico lo asignado, Admin todo)
  se componen con `&`/`|`/`~` sin duplicar queries.
...
Las vistas de `tickets/api/views.py` son `async def`, usando `adrf`
(DRF vanilla no soporta class-based views async de forma estable —
ver `core/async_support/`):
...
6, pero reescribiría toda la capa de negocio y los tests con
`FakeTicketRepository`. El costo de esa migración no se justifica hoy;
...
cp .env.example .env           # editar SECRET_KEY con valor real
...
| `tecnico1` | `Tecnico12345!` | Técnico |
| `usuario1` | `Usuario12345!` | Usuario |
...
Los tests de `TicketService` usan `FakeTicketRepository` (sin DB real),
demostrando que el Service depende de una abstracción, no del ORM.
