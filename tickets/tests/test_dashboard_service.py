...
class TestDashboardSummary:
    def test_counts_by_status_and_priority(self, usuario):
        Ticket.objects.create(
            ...
            priority=TicketPriority.ALTA, status=TicketStatus.ABIERTO, requester=usuario,
        )
        Ticket.objects.create(
            title="b", description="d", category=TicketCategory.SOFTWARE,
            priority=TicketPriority.BAJA, status=TicketStatus.ABIERTO, requester=usuario,
        )
    ...
    def test_avg_resolution_hours_only_counts_closed(self, usuario):
        ...
        closed = Ticket.objects.create(
            ...
            priority=TicketPriority.MEDIA, status=TicketStatus.CERRADO, requester=usuario,
        )
        ...
        Ticket.objects.create(
            ...
            priority=TicketPriority.MEDIA, status=TicketStatus.ABIERTO, requester=usuario,
        )
    ...
    def test_sla_breach_detects_overdue_open_ticket(self, usuario):
        ...
        overdue = Ticket.objects.create(
            ...
            priority=TicketPriority.CRITICA, status=TicketStatus.ASIGNADO, requester=usuario,
        )
    ...
    def test_closed_tickets_never_count_as_sla_breach(self, usuario):
        ...
        old_but_closed = Ticket.objects.create(
            ...
            priority=TicketPriority.CRITICA, status=TicketStatus.CERRADO, requester=usuario,
        )
